...
export default function ReservationManagement() {
  ...
  const handleReservation = async (reservation: Reservation, action: 'approved' | 'rejected') => {
    setLoading(true);
      ...
      try {
        await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/send-reservation-result`, {
          ...
          body: JSON.stringify({
            ...
            fortunetellerName: reservation.fortunetellers.name,
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-xs font-medium">
                      {reservation.fortunetellers.name}
                    </span>
                  </div>
                ...
                {filter === 'pending' && (
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleReservation(reservation, 'approved')}
                      disabled={loading}
                    ...
                    <button
                      onClick={() => handleReservation(reservation, 'rejected')}
                      disabled={loading}
